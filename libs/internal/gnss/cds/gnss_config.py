...
from libs.internal.geo.poly import buffer_bbox_wgs84, geojson_to_bbox, bbox_to_geojson
...
class GnssConfig:
    ...
    def __get_data(self) -> None:
        ...
        # Copernicus CDS API parameters
        self.cds_network_type = self.data_gnss.get(
            "cds_network_type",
            None  # Auto-select based on region: 'epn_repro2' for Europe, 'igs_repro3' for global
        )
        self.cds_variables = self.data_gnss.get(
            "cds_variables",
            ["zenith_total_delay"]  # Default to just ZTD
        )
        self.year = self.data_gnss.get("year", None)  # Optional year override
        self.dates = self.data_gnss.get("dates", None)  # Optional list of specific dates
